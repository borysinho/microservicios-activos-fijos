...
**Cloud**: Amazon AWS  
**CUs**: CU-26 a CU-36 · CU-61 a CU-66
...
## Módulo 5 — Diagnóstico IA (CNN)

### CU-35: Enviar imagen para diagnóstico de estado del activo
...
**Postcondiciones**: La imagen está almacenada en S3. Se inicia el proceso de diagnóstico CNN. Se retorna el diagnóstico con el resultado.

**Flujo principal**:

1. El usuario (o la app móvil) envía la imagen y el `activoId`.
2. El sistema valida: tipo de imagen (JPG, PNG, WEBP), tamaño ≤ 10 MB.
3. El sistema sube la imagen a S3: `diagnosticos/{activoId}/{timestamp}_{nombre}`.
4. El sistema invoca `DiagnosticoService.diagnosticar(imagenBytes, activoId)`.
5. El servicio preprocesa la imagen (redimensionar a 224×224, normalizar).
6. El modelo CNN clasifica la imagen.
7. El sistema persiste el resultado en DynamoDB (tabla `diagnosticos_ia`).
8. Si el estado es `DETERIORADO` con confianza ≥ 0.85: notificar a MS3 (webhook).
9. El sistema retorna el diagnóstico completo.

**Notas técnicas**:

- REST: `POST /api/ia/diagnostico`
- Multipart/form-data: `imagen`, `activoId`
- Si el diagnóstico es crítico (`DETERIORADO`, alta confianza): `POST http://ms3/webhooks/diagnostico-critico`
- CU-79 (Blockchain) se dispara desde MS1 cuando MS1 recibe la notificación de MS3

---

### CU-36: Procesar imagen con CNN y retornar resultado

**Actor**: Sistema (ejecutado internamente por CU-35)  
**Notas técnicas**:

**Modelo CNN `CNN_EstadoActivo`**:
...
- Clases output: `['BUENO', 'DETERIORADO', 'REQUIERE_MANTENIMIENTO']`
- Framework: TensorFlow 2.x / Keras

**Implementación `cnn_estado_activo.py`**:
...
class CNN_EstadoActivo:
    def __init__(self, model_path: str = "modelos/cnn_activo.h5"):
        self.model = tf.keras.models.load_model(model_path)
        self.clases = ['BUENO', 'DETERIORADO', 'REQUIERE_MANTENIMIENTO']
        self.input_size = (224, 224)

    def predecir(self, imagen_bytes: bytes) -> dict:
        imagen = Image.open(io.BytesIO(imagen_bytes)).resize(self.input_size)
        arr = np.array(imagen) / 255.0
        arr = np.expand_dims(arr, axis=0)
        probs = self.model.predict(arr)[0]
        idx = np.argmax(probs)
        return {
            "estado": self.clases[idx],
            "confianza": float(probs[idx]),
            "detalle": {c: float(p) for c, p in zip(self.clases, probs)}
        }
```

**Entrenamiento del modelo (offline)**:

- Dataset: imágenes de activos físicos (equipos, muebles, maquinaria) etiquetadas manualmente
- Mínimo para demo: 300 imágenes por clase (se puede usar dataset simulado)
- Guardar modelo como `cnn_activo.h5` o en formato SavedModel
- Script de entrenamiento en `scripts/train_cnn.py`
