...
3. MS3 dispara el workflow versionado de MS4/N8N en Azure.
4. MS4/N8N identifica el codigo, consulta MS1 y crea la orden o ticket de revision mediante MS3.
5. MS4/N8N verifica documentos del activo en MS2.
6. MS4/N8N solicita a MS3 enviar el correo de confirmacion.
7. MS4/N8N solicita a MS3 responder por WhatsApp al responsable.
...
Regla de arquitectura: frontend, mobile, MS1 y MS2 nunca consumen MS4 directamente. La URL de MS4 se configura solo en MS3 mediante `MS4_N8N_WEBHOOK_URL`.
...
| Disparo N8N desde MS3 | `POST https://<ms4-azure>/webhook/solicitud-revision` | MS3 invoca MS4 usando `MS4_N8N_WEBHOOK_URL`. |
...
Si se usa WhatsApp Cloud API de Meta en lugar de Twilio, configurar `WHATSAPP_PROVIDER=meta`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_TOKEN` y `WHATSAPP_APP_SECRET`.
...
MS1_GRAPHQL_URL=https://<ms1-azure>/graphql
MS2_BASE_URL=https://<ms2-aws>/api
MS3_BASE_URL=https://<ms3-cloud-run>/api
...
El `verify_token` debe coincidir con `WHATSAPP_VERIFY_TOKEN` en MS3.
...
- MS3 desplegado en Cloud Run con `MS4_N8N_WEBHOOK_URL` configurado.
