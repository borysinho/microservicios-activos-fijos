...
- **MS4** → `azure` color `#lightblue` — Azure VM / Docker Compose running N8N
...
participant "MS3 NestJS" as ms3
participant "MS4 N8N" as ms4
...
```
Webhooks Controller → Webhook Service → Notificaciones Service → MS4/N8N Client
```
...
node "Google Cloud Platform" as gcp #lightgreen {
    node "Cloud Run" { artifact "MS3: NestJS" }
}
node "Microsoft Azure" as azure_ms4 #lightblue {
    node "Azure VM / Docker Compose" { artifact "MS4: N8N" }
}
node "Cliente" {
